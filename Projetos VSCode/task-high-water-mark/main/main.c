...
void app_main(void)
{
    ...
    xTaskCreate(
     ...
     while (1)
    {
        ESP_LOGI(TAG1, "Task blink 1 high water mark: %d", uxTaskGetStackHighWaterMark(xTaskBlink1Handle)); 
        ESP_LOGI(TAG1, "Task blink 2 high water mark: %d", uxTaskGetStackHighWaterMark(xTaskBlink2Handle));   
